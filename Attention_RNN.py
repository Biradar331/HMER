...
Modified for CPU-only compatibility
'''
import torch
...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input_a, hidden, encoder_outputs, bb, attention_sum, decoder_attention,
                dense_input, batch_size, h_mask=None, w_mask=None, gpu=None):

        # ✅ CPU-only safe: no gpu splitting
        batch_gpu = batch_size

        # build attention mask (all ones since CPU mode, no image cropping per GPU)
        et_mask = torch.ones(batch_gpu, dense_input, bb)
